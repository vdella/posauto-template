...
\includegraphics[width=\paperwidth,height=\paperheight,keepaspectratio]{Capa/Figuras_Capa/solar-panel.pdf}
    };
\end{tikzpicture}
...
% Nome da revista
\vspace{-3.6cm} % Ajuste a distância vertical para que o nome da revista fique centralizado com a linha horizontal
\begin{center}
    {\norwester\myfontsizeNewstonCapa \textls[40]{DIVULGA}}
\end{center}
\begin{center}
    {\bebasneue\myfontsizeJornalCapa \textls[200]{AUTOMAÇÃO}}
\end{center}
...
\begin{tikzpicture}[remember picture, overlay]
    \node[anchor=north] at ($(current page.north west) + (0.5cm,-1.15cm)$) {
        \rotatebox{90}{\bebasneuebold\myfontsizeTemaCapaVerticalECC \textls[280]{ENERGIAS\, RENOVÁVEIS}}
    };
\end{tikzpicture}
...
% Tema da edição
\hspace{-1.5cm}{\cooperheavy\myfontsizeTema\echoShadowText{baseechoshadow}{baseshadow}{Energias Renováveis:}}

% Logo da UEM Branco
\begin{tikzpicture}[remember picture, overlay]
    % Ajuste a posição da figura conforme necessário
    \node[anchor=south, yshift=17.6cm, xshift=8.6cm] at (current page.south) { % Deslocamento vertical e horizontal na página - altere de acordo com o local em que deseja posicionar a logo
        \includegraphics[width=0.15\textwidth]{Figuras_Capa_ContraCapa/ufsc-bw.png}
    };
\end{tikzpicture}
...
\begin{tikzpicture}[remember picture, overlay]
    \node[anchor=center, inner sep=0pt, outer sep=0pt] at ([shift={(0.0cm,-12.5cm)}] current page.center) {
        \begin{tikzpicture}
             % Texto principal com contorno
            \node[anchor=center, inner sep=0pt, outer sep=0pt] {
                {\cooperheavy\myfontsizeTituloTema \contour{base}{\shortstack[l]{MICRORREDES\\[0.5ex]}}}
            };
        \end{tikzpicture}
    };
\end{tikzpicture}
